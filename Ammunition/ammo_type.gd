...
func fire(pivot: Node2D):
	...
	for offset in angle_offsets:
		...
		_send_visual_pellet(angle, reticle.global_position,stop_pos,self, 5)
...
func _send_visual_pellet(angle: float, start_pos: Vector2,stop_pos:Vector2, ammo_base:AmmoType, innacuracy:float):
